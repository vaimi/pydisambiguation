...
def sim(sense1, sense2, option="path"):
    ...
    if option.lower() in ["path", "path_similarity", 
        ...
        response = similarity_by_path(sense1, sense2, option)
        if response is None:
            return 0
        else:
            return response
                          ...
                          "jcn","jiang-conrath",
                          "lin"]:
        response = similarity_by_infocontent(sense1, sense2, option)
        if response is None:
            return 0
        else:
            return response
...
def max_similarity(context_sentence, ambiguous_word, option="path", 
    ...
    if option in ["res","resnik"]: # lower score = more similar
        result = sorted([(v,k) for k,v in list(result.items())])
    else: # higher score = more similar
        result = sorted([(v,k) for k,v in list(result.items())],reverse=True)
    ##print result
    if best: 
        if result:
            return result[0][1];
